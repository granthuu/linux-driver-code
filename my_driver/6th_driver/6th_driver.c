...
static int sixth_driver_open(struct inode *inode, struct file *filp)
{
	...
	{
	    // down_trylock 在信号量不可获得时，会立即返回一个非零值，永远不会休眠。
...
static int sixth_driver_close(struct inode *inode, struct file *file)
{
...
static ssize_t sixth_driver_read( struct file *file, char *buf, size_t count, loff_t *ppos )
{
...
static struct file_operations third_fops = {
    .owner   = THIS_MODULE,
    .open    = sixth_driver_open,
    .release = sixth_driver_close,
    .read    = sixth_driver_read,
    .poll    = sixth_drv_poll,
...
static int sixth_driver_init(void)
{
    // 1. register char device
    major = register_chrdev(0, "sixth driver", &third_fops);

    // 2. register class, and class device
    third_driver_class = class_create(THIS_MODULE, "sixth driver class");
...
static void sixth_driver_exit(void)
{
    unregister_chrdev(major, "sixth driver");
...
/* 3. GPL license */
module_init(sixth_driver_init);
module_exit(sixth_driver_exit);
MODULE_LICENSE("GPL");
